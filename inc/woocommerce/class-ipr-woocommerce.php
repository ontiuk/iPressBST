...
if ( ! class_exists( 'IPR_WooCommerce' ) ) :
	...
	final class IPR_WooCommerce {
		...
		public function __construct() {
			...
			// Product loop: Wrap cart link & style
			add_filter( 'woocommerce_loop_add_to_cart_args', [ $this, 'add_to_cart_args' ], 10, 2 );

			// Product loop: Add fluid image class
        	add_filter( 'wp_get_attachment_image_attributes', [ $this, 'product_loop_thumbnail_attr' ], 10, 3 );
        	add_filter( 'woocommerce_placeholder_img', [ $this, 'product_loop_placeholder_attr' ], 10, 1 );
			...
			add_filter( 'woocommerce_before_widget_product_list', [ $this, 'widget_product_list' ], 10, 1 );

			// Product widget wrapper, include if price displayed
			// add_action( 'woocommerce_widget_product_item_start', [ $this, 'widget_product_item_start' ], 10, 1 );
			// add_action( 'woocommerce_widget_product_item_end', [ $this, 'widget_product_item_end' ], 10, 1 );
			...
			add_filter( 'woocommerce_billing_fields', [ $this, 'checkout_billing_fields' ], 10, 1 );
			add_filter( 'woocommerce_shipping_fields', [ $this, 'checkout_shipping_fields' ], 10, 1 );
			add_filter( 'woocommerce_checkout_fields', [ $this, 'checkout_order_fields' ], 10, 1 );
			add_filter( 'woocommerce_checkout_fields', [ $this, 'checkout_account_fields' ], 10, 1 );
...
//			add_action( 'woocommerce_after_customer_login_form', [ $this, 'woocommerce_account_container_end' ], 10 );
			...
			// Set addreess fields		
			add_filter( 'woocommerce_default_address_fields', [ $this, 'account_address_fields' ], 10, 1 );
			...
			// Convert form field markup
			add_filter( 'woocommerce_form_field', [ $this, 'form_field_convert' ], 10, 4 );

			// Convery order field markup
			add_filter( 'woocommerce_order_button_html', [ $this, 'form_order_button' ], 10, 1 );
			add_filter( 'woocommerce_pay_order_button_html', [ $this, 'form_order_button' ], 10, 1 );

			// Additional checkout radio
			add_filter( 'woocommerce_form_field_radio', [ $this, 'form_radio_convert' ], 10, 4 );

			// Add Bootstrap field classes to forms
		...
		/**
		 * Add to cart args, bootstrappify
		 *
		 * @param array	$args
		...
		public function add_to_cart_args( $args, $product ) {
			$args['class'] = preg_replace( '/button/', 'button btn-full mt-auto', $args['class'], 1 );
			return $args;
		}

		/**
		 * Add responsive image attribute to thumbnail
		 *
		 * @param array $attr
		 ...
		 * @return array
		 */
		public function product_loop_thumbnail_attr( $attr, $attachment, $size ) {

			if ( $size !== 'woocommerce_thumbnail' ) { return $attr; }
			$attr['class'] .= ' wp-image-' . $attachment->ID . ' d-block img-fluid';

			return $attr;
		}

		/**
		 * Add responsive image attribute to placeholder
		 */
		public function product_loop_placeholder_attr( $str ) {
			return str_replace( 'wp-post-image', 'wp-post-image img-fluid', $str );
		}
		 ...
		 * @return string
		 */	
		public function widget_product_list( $html ) {
			return '<div class="row product-list">';
		}
		 ...
		 * @param array $args
		 */
		public function widget_product_item_start ( $args ) {
		 ...
		 * @param array $args
		 */
		public function widget_product_item_end ( $args ) {
			echo '</div>';
		}
		...
		/**
		 * Checkout billing fields, adds bootstrap classes and translations to checkout form fields
		 * 
		 * @param array $fields
		 * @return array $fields
		 */
		public function checkout_billing_fields( $fields ) {
			...
			$fields['billing_first_name']['class']          = [ 'col-lg-5' ];
			$fields['billing_first_name']['input_class']    = [ 'form-control' ]; 
			$fields['billing_first_name']['label_class']    = [ 'form-label' ];
			$fields['billing_first_name']['placeholder']    = __( 'Enter your first name', 'ipress' );

			$fields['billing_last_name']['class']           = [ 'col-lg-5' ];
			$fields['billing_last_name']['input_class']     = [ 'form-control' ]; 
			$fields['billing_last_name']['label_class']     = [ 'form-label' ];
			$fields['billing_last_name']['placeholder']     = __( 'Enter your last name', 'ipress' );

			$fields['billing_company']['class']             = [ 'col-12' ];
			$fields['billing_company']['input_class']       = [ 'form-control' ]; 
			$fields['billing_company']['label_class']       = [ 'form-label' ];
			$fields['billing_company']['placeholder']       = __( 'Enter your company name', 'ipress' );

			$fields['billing_address_1']['class']           = [ 'col-12' ];
			$fields['billing_address_1']['input_class']     = [ 'form-control' ]; 
			$fields['billing_address_1']['label_class']     = [ 'form-label' ];

			$fields['billing_address_2']['class']           = [ 'col-12' ];
			$fields['billing_address_2']['input_class']     = [ 'form-control' ]; 
			$fields['billing_address_2']['label_class']     = [ 'form-label' ];

			$fields['billing_city']['class']                = [ 'col-4' ];
			$fields['billing_city']['input_class']          = [ 'form-control' ]; 
			$fields['billing_city']['label_class']          = [ 'form-label' ];
			$fields['billing_city']['placeholder']          = __( 'Enter your town/city', 'ipress' );

			$fields['billing_postcode']['class']            = [ 'col-lg-4' ];
			$fields['billing_postcode']['input_class']      = [ 'form-control' ]; 
			$fields['billing_postcode']['label_class']      = [ 'form-label' ];
			$fields['billing_postcode']['placeholder']      = __( 'Enter your postcode', 'ipress' );

			$fields['billing_state']['class']               = [ 'col-lg-4' ];
			$fields['billing_state']['input_class']         = [ 'form-control' ]; 
			$fields['billing_state']['label_class']         = [ 'form-label' ];
			$fields['billing_state']['placeholder']         = __( 'Enter your county', 'ipress' );

			$fields['billing_phone']['class']               = [ 'col-lg-6' ];
			$fields['billing_phone']['input_class']         = [ 'form-control' ]; 
			$fields['billing_phone']['label_class']         = [ 'form-label' ];
			$fields['billing_phone']['placeholder']         = __( 'Enter your phone no.', 'ipress' );
			$fields['billing_phone']['required'] 			= false;
			$fields['billing_phone']['priority']         	= 110;

			$fields['billing_email']['class']               = [ 'col-lg-6' ];
			$fields['billing_email']['input_class']         = [ 'form-control' ]; 
			$fields['billing_email']['label_class']         = [ 'form-label' ];
			$fields['billing_email']['placeholder']         = __( 'Enter your email address', 'ipress' );
			$fields['billing_email']['priority']         	= 120;

			$fields['billing_country']['class']      		= [ 'col-6 d-none' ];
			$fields['billing_country']['input_class']       = [ 'form-control' ]; 
			$fields['billing_country']['label_class']       = [ 'form-label' ];
			$fields['billing_country']['placeholder']       = __( 'Select your country', 'ipress' );
		...
		/**
		 * Checkout shipping fields, adds bootstrap classes and translations to checkout form fields
		 *
		 * @param array $fields
		 * @return array $fields
		 */
		public function checkout_shipping_fields( $fields ) {
			...
			$fields['shipping_first_name']['class']         = [ 'col-lg-5' ];
			$fields['shipping_first_name']['input_class']   = [ 'form-control' ]; 
			$fields['shipping_first_name']['label_class']   = [ 'form-label' ];
			$fields['shipping_first_name']['placeholder']   = __( 'Enter your first name', 'ipress' );

			$fields['shipping_last_name']['class']          = [ 'col-lg-5' ];
			$fields['shipping_last_name']['input_class']    = [ 'form-control' ]; 
			$fields['shipping_last_name']['label_class']    = [ 'form-label' ];
			$fields['shipping_last_name']['placeholder']    = __( 'Enter your last name', 'ipress' );

			$fields['shipping_company']['class']            = [ 'col-12' ];
			$fields['shipping_company']['input_class']      = [ 'form-control' ]; 
			$fields['shipping_company']['label_class']      = [ 'form-label' ];
			$fields['shipping_company']['placeholder']      = __( 'Enter your company name', 'ipress' );

			$fields['shipping_address_1']['class']          = [ 'col-12' ];
			$fields['shipping_address_1']['input_class']    = [ 'form-control' ]; 
			$fields['shipping_address_1']['label_class']    = [ 'form-label' ];

			$fields['shipping_address_2']['class']          = [ 'col-12' ];
			$fields['shipping_address_2']['input_class']    = [ 'form-control' ]; 
			$fields['shipping_address_2']['label_class']    = [ 'form-label' ];

			$fields['shipping_city']['class']               = [ 'col-lg-4' ];
			$fields['shipping_city']['input_class']         = [ 'form-control' ]; 
			$fields['shipping_city']['label_class']         = [ 'form-label' ];
			$fields['shipping_city']['placeholder']         = __( 'Enter your town/city', 'ipress' );

			$fields['shipping_postcode']['class']           = [ 'col-lg-4' ];
			$fields['shipping_postcode']['input_class']     = [ 'form-control' ]; 
			$fields['shipping_postcode']['label_class']     = [ 'form-label' ];
			$fields['shipping_postcode']['placeholder']     = __( 'Enter your postcode', 'ipress' );

			$fields['shipping_state']['class']              = [ 'col-lg-4' ];
			$fields['shipping_state']['input_class']        = [ 'form-control' ]; 
			$fields['shipping_state']['label_class']        = [ 'form-label' ];
			$fields['shipping_state']['placeholder']        = __( 'Enter your county', 'ipress' );

			$fields['shipping_country']['class']      		= [ 'col-6 d-none' ];
			$fields['shipping_country']['input_class']      = [ 'form-control' ]; 
			$fields['shipping_country']['label_class']      = [ 'form-label' ];
			$fields['shipping_country']['placeholder']      = __( 'Select your country', 'ipress' );
		...
		/**
		 * Checkout Order fields, adds bootstrap classes to form fields
		 * 
		 * @param array $fields
		 * @return array $fields
		 */
		public function checkout_order_fields( $fields ) {

			$fields['order']['order_comments']['class']         = [ 'form-group notes col-12' ];
		...
		/**
		 * Checkout Account fields, adds bootstrap classes to form fields
		 * 
		 * @param array $fields
		 * @return array $fields
		 */
		public function checkout_account_fields( $fields ) {

			// Not set? Depends on the checkout settings
			if ( ! isset( $fields['account'] ) || empty( $fields['account'] ) ) { return $fields; }
		   	
			$fields['account']['account_username']['class']         = [ 'col-12' ];
			$fields['account']['account_username']['input_class']   = [ 'form-control' ]; 
			$fields['account']['account_username']['label_class']   = [ 'form-label' ];

			$fields['account']['account_password']['class']         = [ 'col-lg-6' ];
			$fields['account']['account_password']['input_class']   = [ 'form-control' ]; 
			$fields['account']['account_password']['label_class']   = [ 'form-label' ];

			$fields['account']['account_password-2']['class']       = [ 'col-lg-6' ];
			$fields['account']['account_password-2']['input_class'] = [ 'form-control' ]; 
			$fields['account']['account_password-2']['label_class'] = [ 'form-label', 'hidden' ];
		...
		/**
		 * Checkout address fields, adds bootstrap classes to form fields
		 * 
		 * @param array $fields
		 * @return array $fields
		 */
		public function account_address_fields( $fields ) {

			// Modify fields: format
			$fields['first_name']['class'] 	= [ 'form-group' ];
			$fields['last_name']['class'] 	= [ 'form-group' ];
			$fields['company']['class'] 	= [ 'form-group' ];
			$fields['country']['class'] 	= [ 'form-group', 'address-field', 'update_totals_on_change' ];
			$fields['address_1']['class'] 	= [ 'form-group', 'address-field' ];
			$fields['address_2']['class'] 	= [ 'form-group', 'address-field' ];
			$fields['city']['class'] 		= [ 'form-group', 'address-field' ];
			$fields['state']['class'] 		= [ 'form-group', 'address-field' ];
			$fields['postcode']['class'] 	= [ 'form-group', 'address-field' ];
		...
		/**
		 * Bootstrap input field, translates default P to div and removes some unnecessary default formatting
		 *
		 * @param string $field
		 * @param string $key
		 * @param array $args
		 ...
		 * @return string
		 */
		public function form_field_convert( $field, $key, $args, $value ) {
			return str_replace( [ '<p', '</p>' ], [ '<div', '</div>' ], str_replace( [ 'form-row-first', 'form-row-last', 'form-row-wide', 'form-row' ], '', $field ) );
		}

		/**
		 * Checkout payment button, convert to bootstrap button markup
		 *
		 * @param string $html
		 * @return string
		 */
		public function form_order_button( $text ) {   
			return str_replace( 'alt', 'btn btn-primary btn-wide btn-uppercase', str_replace( 'class="button', 'class="', $text ) );
		}

		/**
		 * Bootstrap radio field, translates default P to div and adds some bootstrap formatting
		 *
		 * @param string $field
		 * @param string $key
		 * @param array $args
		 ...
		 * @return string $field
		 */
		public function form_radio_convert( $field, $key, $args, $value ) {

			// Process radio fields
			$field = str_replace( '<input', '<div class="form-check"><input', str_replace( '</label>', '</label></div>', $field ) );
			$field = str_replace( [ '<p', '</p>' ], [ '<div', '</div>' ], str_replace( [ 'form-row' ], '', $field ) );

			return $field;
		}

		/**
		 * Bootstrap translate generif form field types
		 *
		 * @param array $args
		 * @param string $key
